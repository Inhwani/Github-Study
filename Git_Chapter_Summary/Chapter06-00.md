...
브랜치 이름 대신 커밋 해시키를 사용하여 체크아웃도 가능.
...
이는 마지막 커밋인 HEAD를 기준으로 1단계의 커밋 상태로 이동함

# 6.8.4 돌아오기

커밋 해시키 또는 HEAD를 사용하여 과거의 커밋으로 체크아웃했다면 다시 현재 시점으로 돌아와야 함

간단하게 돌아오는 방법은 대시를 (-) 사용하는 것
...
원격 저장소에 등록된 브랜치가 자동으로 로컬 저장소를 만들지 않음.

별도 명령을 실행하여 저장소를 동기화해야 함.
...
로컬 저장소의 브랜치 이름과 원격 저장소의 브랜치 이름은 동일하게 사용함.

브랜치를 직접 수동으로 지정할 대는 콜론으로 구분.
